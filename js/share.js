...
function saveStateToStorage(selections) {
  if (typeof storageSetEstimateDraft !== "function") return;
  storageSetEstimateDraft({
...
function loadStateFromStorage() {
  if (typeof storageGetEstimateDraft !== "function") return null;
  return storageGetEstimateDraft();
}

/** LocalStorageの状態を削除 */
function clearStateFromStorage() {
  if (typeof storageClearEstimateDraft !== "function") return;
  storageClearEstimateDraft();
}

function getShareUiText() {
  const lang = typeof getCurrentLang === "function" ? getCurrentLang() : "ja";
  return lang === "en"
    ? { copied: "State URL copied", prompt: "Copy this state URL:" }
    : { copied: "選択状態URLをコピーしました", prompt: "この選択状態URLをコピーしてください:" };
}
...
function initShareButton() {
  ...
  btn.addEventListener("click", async () => {
    ...
    const text = getShareUiText();

    if (ok) {
      showShareFeedback(btn, text.copied);
    } else {
      // コピー失敗時はpromptで表示
      window.prompt(text.prompt, url);
    }
